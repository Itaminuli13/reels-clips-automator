import os
import random
import time
from datetime import datetime
from gtts import gTTS
from moviepy.editor import VideoFileClip, AudioFileClip
from instagrapi import Client

cl = Client()
cl.delay_range = [3, 10]
cl.load_settings("session.json")
try:
    cl.get_timeline_feed()
    print("لاگین شد")
except:
    exit()

captions = [
    "AI is taking over 2026! #AI #Tech #Viral #FYP",
    "ChatGPT just got 100x smarter! #AI #ChatGPT",
    "Grok 4 dropped — Elon went crazy! #Grok #ElonMusk",
    "Neuralink first human trials started! #Neuralink",
    "Tesla Bot walks better than you! #Tesla #AI",
    "Quantum computing is here! #Tech #2025",
    "Web3 will replace everything! #Crypto #Web3"
]

def post():
    caption = random.choice(captions)
    ts = datetime.now().strftime("%H-%M")
    audio = f"audio_{ts}.mp3"
    output = f"reel_{ts}.mp4"

    print(f"ساخت ریلز: {caption}")

    gTTS(caption, lang='en').save(audio)

    bg = VideoFileClip("bg.mp4").subclip(0, 20)  # از فایل آپلود شده
    voice = AudioFileClip(audio)
    final = bg.set_audio(voice)
    final = final.subclip(0, min(15, voice.duration + 2))

    final.write_videofile(output, fps=30, codec="libx264", audio_codec="aac",
                          preset="ultrafast", threads=2, logger=None, verbose=False)

    try:
        cl.clip_upload(output, caption=caption)
        print(f"ریلز خفن رفت بالا! {ts}")
    except Exception as e:
        print("آپلود نشد:", e)

    for f in [audio, output]:
        if os.path.exists(f): os.remove(f)

if __name__ == "__main__":
    print("ربات شروع شد — نسخه نهایی با بک‌گراند خفن")
    while True:
        try:
            post()
        except: pass
        print("۴ ساعت دیگه...")
        time.sleep(4 * 60 * 60)
